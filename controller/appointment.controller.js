...
export const createAppointment = async (req, res) => {
    ...
    try {
        ...
        if (
            ...
            return res.status(400).json({ success: false, message: "All fields are required", formErrors: { global: "All fields are required" } });
        }
        ...
        if (!doctor.status && req.user.role === 'patient') {
            return res.status(400).json({
                ...
                formErrors: { doctor: "Doctor is inactive" }
            });
        }
        ...
        if (!hospital.status) {
            return res.status(400).json({
                ...
                formErrors: { hospital: "Hospital is inactive" }
            })
        }
